...
SELECT '1234', 1500.00, 700.00, 900.00 WHERE NOT EXISTS (SELECT 1 FROM tb_crgd_account WHERE account_id = '1234');

INSERT INTO tb_crgd_account (account_id, amount_cash, amount_meal, amount_food)
SELECT '5678', 1200.00, 600.00, 800.00 WHERE NOT EXISTS (SELECT 1 FROM tb_crgd_account WHERE account_id = '5678');

-- Inserindo dados na tabela tb_merchant
INSERT INTO tb_merchant (merchant_name, mcc)
SELECT 'UBER TRIP                   SAO PAULO BR', '4121'
WHERE NOT EXISTS (SELECT 1 FROM tb_merchant WHERE merchant_name = 'UBER TRIP                   SAO PAULO BR');

INSERT INTO tb_merchant (merchant_name, mcc)
SELECT 'UBER EATS                   SAO PAULO BR', '5812'
WHERE NOT EXISTS (SELECT 1 FROM tb_merchant WHERE merchant_name = 'UBER EATS                   SAO PAULO BR');

INSERT INTO tb_merchant (merchant_name, mcc)
SELECT 'PAG*JoseDaSilva          RIO DE JANEI BR', '4829'
WHERE NOT EXISTS (SELECT 1 FROM tb_merchant WHERE merchant_name = 'PAG*JoseDaSilva          RIO DE JANEI BR');

INSERT INTO tb_merchant (merchant_name, mcc)
SELECT 'PICPAY*BILHETEUNICO           GOIANIA BR', '4111'
WHERE NOT EXISTS (SELECT 1 FROM tb_merchant)